...
import { useProfessionalProfileBySurgeonName } from '@/hooks/useProfessionalProfile';
import { useDefaultProcedureCodes, useSaveDefaults } from '@/hooks/useDefaultProcedureCodes';
...
export function GenerateDocumentDialog({ open, onClose, patient }: Props) {
  ...
  const { data: surgeonProfile } = useProfessionalProfileBySurgeonName(patient?.surgeon ?? null);
  const signatureInfo = useMemo(() => ({
    name: patient?.surgeon ?? '',
    crm: surgeonProfile?.crm ?? '',
    crmUf: surgeonProfile?.crm_uf ?? '',
    rqe: surgeonProfile?.rqe ?? '',
    title: surgeonProfile?.signature_title ?? '',
  }), [patient?.surgeon, surgeonProfile]);

  const { data: defaultsBundle } = useDefaultProcedureCodes(
    patient?.procedure ?? null,
    patient?.surgeon ?? null,
    patient?.concierge ?? null,
  );
  ...
  }, [open, type, patient?.id, selectedTemplate?.id, defaultsBundle]);

  const previewBody = useMemo(() => {
    if (!structured) return '';
    switch (structured.kind) {
      case 'surgical_request': return buildSurgicalRequestHtml(structured.data, signatureInfo);
      case 'prescription': return buildPrescriptionHtml(structured.data, signatureInfo);
      case 'medical_certificate': return buildMedicalCertificateHtml(structured.data, signatureInfo);
      case 'report': return buildReportHtml(structured.data, signatureInfo);
      case 'budget': return buildBudgetHtml(structured.data, signatureInfo);
    }
  }, [structured, signatureInfo]);
  ...
  const handleSaveDefaultsConfirm = async (chosen: { surgeon: boolean; concierge: boolean }) => {
    ...
    if (structured?.kind === 'surgical_request' && patient?.procedure) {
      const sd = structured.data;
      try {
        if (chosen.surgeon && patient.surgeon) {
          await saveDefaults.mutateAsync({ procedure: patient.procedure, scope: 'surgeon', scope_owner: patient.surgeon, data: sd });
        }
        if (chosen.concierge && patient.concierge) {
          await saveDefaults.mutateAsync({ procedure: patient.procedure, scope: 'concierge', scope_owner: patient.concierge, data: sd });
        }
      } catch (e) {
        console.warn('saveDefaults', e);
      }
    }
    runGenerate();
  };
  ...
  return (
    <>
      <Dialog open={open} onOpenChange={(o) => !o && handleClose()}>
        <DialogContent className="max-w-5xl max-h-[92vh] overflow-hidden p-0 flex flex-col">
          ...
          <DialogFooter className="px-6 py-3 border-t border-border shrink-0">
            ...
            <Button onClick={handleGenerateClick} disabled={generate.isPending || !structured}>
      ...
      {structured?.kind === 'surgical_request' && (
        <SaveDefaultsDialog
          open={saveDefaultsOpen}
          procedure={patient?.procedure ?? ''}
          surgeon={patient?.surgeon ?? null}
          concierge={patient?.concierge ?? null}
          data={structured.data}
          onConfirm={handleSaveDefaultsConfirm}
          onCancel={() => setSaveDefaultsOpen(false)}
        />
      )}
    </>
  );
